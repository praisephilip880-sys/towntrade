...
function createRemoteDb() {
  ...
  // Runs inside the worker: owns the async client, executes queries, replies
  // through the port and bumps the SharedArrayBuffer to wake the main thread
  // (which is synchronously waiting for the answer).
  // ESM worker (forced via type: 'module' so behaviour is identical on every
  // Node version and on Vercel). Runs the async client and answers the main
  // thread synchronously through the port + SharedArrayBuffer signal.
  const WORKER_SOURCE = `
    ...
    workerData.port.on('message', async (msg) => {
      ...
      try { workerData.port.postMessage({ id: msg.id, ok, value, error }); } catch {}
      try { Atomics.add(sig, 0, 1); Atomics.notify(sig, 0); } catch {}
    });
  `;
  ...
  const worker = new Worker(WORKER_SOURCE, {
    eval: true,
    type: 'module',
    workerData: { port: port2, sig },
    transferList: [port2],
  });
  try { port2.close(); } catch { /* already transferred to the worker */ }
  worker.on('error', (err) => console.error('[TownTrade] cloud database worker error:', err.message || err));
