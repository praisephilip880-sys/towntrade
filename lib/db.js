...
function createRemoteDb() {
  ...
  // A real file (not an eval'd string) so Next.js/webpack traces its imports
  // and ships @libsql/client/web + the ws polyfill with the server bundle.
  const { port1, port2 } = new MessageChannel();
  const sab = new SharedArrayBuffer(4);
  const sig = new Int32Array(sab);
  const worker = new Worker(new URL('./db.worker.cjs', import.meta.url), {
    type: 'commonjs',
