...
import { svgDataUrl } from './seedImages';
// Where the SQLite file lives. Override with DATA_DIR when the database must
// live on a mounted persistent disk (e.g. Render/Railway attach a volume here).
const DATA_DIR = process.env.DATA_DIR ? path.resolve(process.env.DATA_DIR) : path.join(process.cwd(), 'data');
if (!fs.existsSync(DATA_DIR)) {
    fs.mkdirSync(DATA_DIR, { recursive: true });
}
const globalForDb = globalThis;
export const db = globalForDb.__towntradeDb ??
    (() => {
        ...
        globalForDb.__towntradeDb = instance;
        return instance;
    })();

/**
 * Node's built-in SQLite has no .transaction() helper (unlike better-sqlite3).
 * Add one so callers can wrap multi-statement work in BEGIN/COMMIT/ROLLBACK.
 */
