...
const WORKER_SOURCE = `
  ...
  // An open hrana transaction: keeps ONE stream alive across every statement
  // between BEGIN and COMMIT/ROLLBACK. Without this, each execute call opens
  // and closes its own stream, so BEGIN/COMMIT never span calls and every
  // db.transaction() on the cloud backend would fail.
  let tx = null;
  ...
  async function runQuery(msg) {
    ...
    if (msg.kind === 'rollback') {
      if (tx) { try { await tx.rollback(); } catch { /* already closed */ } tx = null; }
      return { _tx: true };
    }
    if (tx) {
      if (msg.kind === 'execute') return await tx.execute({ sql: msg.sql, args: msg.args });
      return await tx.executeMultiple(msg.sql);
    }
    if (msg.kind === 'execute') {
  ...
  workerData.port.on('message', async (msg) => {
    ...
    try {
      ...
      // A half-open transaction is discarded; the caller will roll back.
      tx = null;
...
function createRemoteDb() {
  ...
  const adapter = {
    ...
    exec(sql) {
      // Transaction control must go over a single persistent stream (the
      // worker's 'begin'/'commit'/'rollback' kinds). Everything else runs as
      // executeMultiple (schema/migrations).
      const s = String(sql).trim().toUpperCase();
      if (s === 'BEGIN') { remote('begin', '', []); return; }
      if (s === 'COMMIT') { remote('commit', '', []); return; }
      if (s === 'ROLLBACK') { remote('rollback', '', []); return; }
