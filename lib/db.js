...
// Spawn the worker by filesystem path (a plain string) instead of the
// `new Worker(new URL(...))` pattern: webpack rewrites the URL form into a
// special worker chunk that crashes when the module is executed at build time
// (page-data collection with TURSO_* set, as on Vercel). The plain path is
// left untouched, and outputFileTracingIncludes in next.config.js guarantees
// the file ships with the serverless function.
const WORKER_PATH = path.join(process.cwd(), 'lib', 'db.worker.cjs');
...
function createRemoteDb() {
  ...
  function ensureWorker() {
    ...
    worker = new Worker(WORKER_PATH, {
      ...
      // or --experimental flags): they can break file-based worker startup.
      execArgv: [],
